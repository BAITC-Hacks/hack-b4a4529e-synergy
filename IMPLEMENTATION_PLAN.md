...
4. **Attachments.** Signature/MIME/parser validation precedes submission. Images use image inputs; PDF and DOC/DOCX use native Responses file inputs. XLS/XLSX and CSV are parsed deterministically on the server, matched in chunks and reconciled against every nonempty data row. Limits: five files, 10 MB each, 20 MB total, 1,000 rows per sheet/CSV, 100 PDF pages. Corrupt/encrypted files receive clear errors. Office image limitations are shown. Structured rows preserve source references, quantities, candidate IDs and resolution state; original bytes are not retained in session history. Users explicitly select batches of at most 50 lines; all cart additions require fresh confirmation.
